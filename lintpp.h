...
/** \brief Класс целых чисел произвольной точности.
...
public:
	...
	*/
	...
	/** \brief Базовый класс генератора случайных чисел.
	...
	*/
	...
	public:
		...
		/** \brief Создать генератор с заданной максимальной длиной чисел.
			...
			Генератор вычисляет только положительные числа из диапазона [0, 10^(length + 1) - 1] - числа,
			количество разрядов в которых не превышает length (в отличие от функции #rand, которая создает
			числа со знаком).
		...
		*/
		...
		/** \brief Выдать очередное случайное число.
			\return Очередное случаяное число.
		*/
		linteger get();
